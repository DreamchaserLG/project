	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
			...
			:default-expand-all="isExpandAll" ref="tableRef">
				...
				<el-table-column prop="order_number" @sort-change="$sortChange" label="订单编号" 				v-if="$check_field('get','order_number')" min-width="300">
					...
					<el-table-column prop="confirm_time" @sort-change="$sortChange" label="确认时间" 				v-if="$check_field('get','confirm_time')" min-width="300">
						<template slot-scope="scope">
									                	{{ $toTime(scope.row["confirm_time"],"yyyy-MM-dd hh:mm:ss") }}
																		</template>
					</el-table-column>
					<el-table-column prop="number_of_attendees" @sort-change="$sortChange" label="到场人数" 				v-if="$check_field('get','number_of_attendees')" min-width="300">
					<template slot-scope="scope">
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/travel_confirmation/table','set') || $check_action('/travel_confirmation/view','set') || $check_action('/travel_confirmation/view','get')
						" >
