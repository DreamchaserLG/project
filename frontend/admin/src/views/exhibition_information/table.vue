	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
			...
			:default-expand-all="isExpandAll" ref="tableRef">
				...
				<el-table-column prop="exhibitionconvention_number" @sort-change="$sortChange" label="会展编号" 				v-if="$check_field('get','exhibitionconvention_number')" min-width="300">
					...
					<el-table-column prop="exhibition_and_convention_types" @sort-change="$sortChange" label="会展类型" 				v-if="$check_field('get','exhibition_and_convention_types')" min-width="300">
					<template slot-scope="scope">
					...
					<el-table-column prop="exhibition_status" @sort-change="$sortChange" label="会展状态" 				v-if="$check_field('get','exhibition_status')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["exhibition_status"] }}
									</template>
					</el-table-column>
					<el-table-column prop="exhibition_poster" @sort-change="$sortChange" label="会展海报" 				v-if="$check_field('get','exhibition_poster')" min-width="300">
						<template slot-scope="scope">
					...
					<el-table-column prop="introduction_document" @sort-change="$sortChange" label="介绍文档" 				v-if="$check_field('get','introduction_document')" min-width="300">
						<template slot-scope="scope">
					<a v-if="scope.row['introduction_document']" :href="$fullUrl(scope.row['introduction_document'])" target="_blank" style="color: rgb(64, 158, 255);margin-right: 10px;">点击下载</a>
					<a v-if="showPreview(scope.row['introduction_document'])" @click="previewFile(scope.row['introduction_document'])" style="color: rgb(64, 158, 255);">预览</a>
				</template>
					</el-table-column>
					<el-table-column prop="fee_standards" @sort-change="$sortChange" label="费用标准" 				v-if="$check_field('get','fee_standards')" min-width="300">
					<template slot-scope="scope">
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/exhibition_information/table','set') || $check_action('/exhibition_information/view','set') || $check_action('/exhibition_information/view','get')
						" >
				...
				<template slot-scope="scope">
					...
					v-if="$check_action('/exhibition_information/table','set') || $check_action('/exhibition_information/view','set') || $check_action('/exhibition_information/view','get')"
						...
						<router-link v-if="$check_comment('/exhibition_information/table')" class="e-button el-button--small is-plain el-button--primary"
						...
						<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="changeSigning(scope.row, scope.$index)" v-if="$check_option('/exhibition_information/table', 'examine')  ">
						<span>审核</span>
					</el-button>
				</div>
				</template>
