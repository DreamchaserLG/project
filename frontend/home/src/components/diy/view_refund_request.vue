	...
	<el-main class="bg edit_wrap">
		...
		<el-row class="row_ce">
				...
				<el-form-item label="订单编号" prop="order_number">
							...
							v-if="(form['refund_request_id'] && $check_field('set','order_number')) || (!form['refund_request_id'] && $check_field('add','order_number'))" :disabled="disabledObj['order_number_isDisabled']">
									</el-input>
				...
				<el-form-item label="主办用户" prop="host_user">
													<el-select v-if="(form['refund_request_id'] && $check_field('set','host_user')) || (!form['refund_request_id'] && $check_field('add','host_user'))" id="host_user" v-model="form['host_user']" :disabled="disabledObj['host_user_isDisabled']">
				...
				<el-form-item label="展位价格" prop="booth_prices">
						...
						v-if="(form['refund_request_id'] && $check_field('set','booth_prices')) || (!form['refund_request_id'] && $check_field('add','booth_prices'))" :disabled="disabledObj['booth_prices_isDisabled']"></el-input-number>
				...
				<el-form-item label="报名用户" prop="enrolled_user">
													<el-select v-if="(form['refund_request_id'] && $check_field('set','enrolled_user')) || (!form['refund_request_id'] && $check_field('add','enrolled_user'))" id="enrolled_user" v-model="form['enrolled_user']" :disabled="disabledObj['enrolled_user_isDisabled']">
				...
				<el-form-item label="用户姓名" prop="user_name">
							...
							v-if="(form['refund_request_id'] && $check_field('set','user_name')) || (!form['refund_request_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']">
									</el-input>
				...
				<el-form-item label="用户手机" prop="users_mobile_phone">
							...
							v-if="(form['refund_request_id'] && $check_field('set','users_mobile_phone')) || (!form['refund_request_id'] && $check_field('add','users_mobile_phone'))" :disabled="disabledObj['users_mobile_phone_isDisabled']">
									</el-input>
					<div v-else-if="$check_field('get','users_mobile_phone')">
						{{form['users_mobile_phone']}}
																...
																<el-col v-if="$check_field('get','application_time') || $check_field('add','application_time') || $check_field('set','application_time')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="申请时间" prop="application_time">
								<el-date-picker :disabled="disabledObj['application_time_isDisabled']" v-if="(form['refund_request_id'] && $check_field('set','application_time')) || (!form['refund_request_id'] && $check_field('add','application_time'))" id="application_time"
					...
					<div v-else-if="$check_field('get','application_time')">{{form['application_time']}}</div>
																...
																<el-col v-if="$check_field('get','reason_for_application') || $check_field('add','reason_for_application') || $check_field('set','reason_for_application')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="申请理由" prop="reason_for_application">
								<el-input type="textarea" id="reason_for_application" v-model="form['reason_for_application']" placeholder="请输入申请理由"
						v-if="(form['refund_request_id'] && $check_field('set','reason_for_application')) || (!form['refund_request_id'] && $check_field('add','reason_for_application'))" :disabled="disabledObj['reason_for_application_isDisabled']"></el-input>
				...
				<el-form-item v-if="$check_action('/refund_request/view','set') || $check_action('/refund_request/view','add')">
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/refund_request/add?",
				url_set: "~/api/refund_request/set?",
				url_get_obj: "~/api/refund_request/get_obj?",
				url_get_list: "~/api/refund_request/get_list?",
				url_upload: "~/api/refund_request/upload?",
		...
		toFixed: function(num){
			...
			async get_obj_after(json, func){
				if (json.result && json.result.obj) {
					const obj = json.result.obj;
																																																					...
																																																					if(json.result.obj["application_time"]=="0000-00-00 00:00:00"){
				  json.result.obj["application_time"] = null;
				}
				if(parseInt(json.result.obj["application_time"]) > 9999){
					json.result.obj["application_time"] = this.$toTime(parseInt(json.result.obj["application_time"]),"yyyy-MM-dd hh:mm:ss")
				}
				...
				if(!this.form.refund_request_id){
				...
				return ret;
					...
					submit_after(json, func) {
						...
						if(!this.form.refund_request_id){
								...
								title: '退款申请',
								type: '消息',
								content: this.$store.state.user.username + '-' + this.$store.state.user.nickname + '提交了一条退款申请数据',
								state: 1,
								user_id:'9999'
							}
			...
			async submit_check(param) {
					...
					return "申请时间不能为空";
																												...
																												var reason_for_application_SensitiveWords = await this.filterSensitiveWords(param.reason_for_application)
			if(reason_for_application_SensitiveWords.length > 0){
				return '申请理由中包含敏感词: ' + reason_for_application_SensitiveWords.join(',')
			}
								return null;
			},
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/refund_request/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/refund_request/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/refund_request/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/refund_request/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/refund_request/view','get');
