<template>
	<div class="diy_details_box diy_div_travel_confirmation">
		<div class="warp diy_warp">
						...
						<!-- 图片 -->
							...
							<div class="row_2" :class="{flex_row2: !imgList.length}">
								...
								<div class="diy_field diy_music" :class="{flex_row2_item_text:imgList.length}" v-else-if="item.type == '文件'">
									...
									<a
										@click="previewFile(obj[item.name])"
								...
								<div class="diy_field diy_date" :class="{flex_row2_item_text:imgList.length}" v-else-if="item.type == '日期' || item.type == '日后'">
									<span>
										{{ item.hide_str ? $hide_str($toTime(obj[item.name] ,"yyyy-MM-dd")) : $toTime(obj[item.name] ,"yyyy-MM-dd") }}
									</span>
								</div>
								<div class="diy_field diy_time" :class="{flex_row2_item_text:imgList.length}" v-else-if="item.type == '时间'">
									<span>
										{{ item.hide_str ? $hide_str($toTime(obj[item.name] ,"hh:mm:ss")) : $toTime(obj[item.name] ,"hh:mm:ss") }}
									</span>
								</div>
								<div class="diy_field diy_datetime" :class="{flex_row2_item_text:imgList.length}" v-else-if="item.type == '日长'">
									<span>
										{{ item.hide_str ? $hide_str($toTime(obj[item.name] ,"yyyy-MM-dd hh:mm:ss")) : $toTime(obj[item.name] ,"yyyy-MM-dd hh:mm:ss") }}
									</span>
								</div>
								...
								<div class="diy_field diy_multiple_img" :class="{flex_row2_item_text:imgList.length}" v-else-if="item.type == '图集'">
									<div>
										<el-image class="diy_multiple_img_item"
											:src="img" v-for="img in obj[item.name]" :key="img" :preview-src-list="obj[item.name]">
								...
								<div class="diy_field diy_text" :class="{flex_row2_item_text:imgList.length}" v-else>
									<span>
										{{ item.hide_str ? $hide_str(obj[item.name]) : obj[item.name] }} {{ item.units }}
									</span>
								</div>
			...
			<div class="rich_text" v-for="(item,index) in richList" :key="item+index" v-show="$check_field('get',item.name)">
						...
						<div class="diy_title">
							...
							<div v-if="item.type == '视频'" class="diy_html" style="display: flex;justify-content: center;">
								<video style="width: 100%;" :src="$fullUrl(obj[item.name])" controls></video>
							</div>
							<div v-else class="diy_html" v-html="obj[item.name]" ></div>
						</div>
						</div>
			</div>
