         ...
         <el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
                ...
                <el-button @click="reset()" style="margin-right: 74px;" class="search_btn_reset">重置</el-button>
                                            <el-button v-if="$check_option('/data_statistics/table','export_db')" @click="export_db('dataTable')"  class="derive" style="margin-right: 155px;">导出</el-button>
                                                      <el-button v-if="$check_action('/data_statistics/table','add') || $check_action('/data_statistics/view','add')" @click="$router.push('./view?')" class="add">添加</el-button>
	    ...
	    <el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable" row-key="data_statistics_id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }"
			:default-expand-all="isExpandAll" ref="tableRef">
	                <el-table-column type="selection" tooltip-effect="dark" width="55">
                                                  ...
                                                  <el-table-column prop="statistics_number" @sort-change="$sortChange" label="统计编号"                                v-if="$check_field('get','statistics_number')" min-width="200">
                  {
                    <template slot-scope="scope">
                                          {{ scope.row['statistics_number'] }}
                                              ...
                                              <el-table-column prop="statistics_type" @sort-change="$sortChange" label="统计类型"                                v-if="$check_field('get','statistics_type')" min-width="200">
                  {
                    <template slot-scope="scope">
                                          {{ scope.row['statistics_type'] }}
                                              ...
                                              <el-table-column prop="statistical_indicators" @sort-change="$sortChange" label="统计指标"                                v-if="$check_field('get','statistical_indicators')" min-width="200">
                  {
                    <template slot-scope="scope">
                                          {{ scope.row['statistical_indicators'] }}
                                              ...
                                              <el-table-column prop="statistics_date" @sort-change="$sortChange" label="统计日期"                                v-if="$check_field('get','statistics_date')" min-width="200">
                                      <template slot-scope="scope">
                                          {{ $toTime(scope.row["statistics_date"],"yyyy-MM-dd") }}
                                                            </template>
                                </el-table-column>
                                              <el-table-column prop="statistical_results" @sort-change="$sortChange" label="统计结果"                                v-if="$check_field('get','statistical_results')" min-width="200">
                  {
                    <template slot-scope="scope">
      ...
      <el-table-column sortable prop="update_time" label="更新时间" min-width="200">
        <template slot-scope="scope">
          {{ $toTime(scope.row["update_time"],"yyyy-MM-dd hh:mm:ss") }}
                        ...
                        <el-table-column label="操作" min-width="120" v-if="$check_action('/data_statistics/table','set') || $check_action('/data_statistics/view','set') || $check_action('/data_statistics/view','get')
					 ">
