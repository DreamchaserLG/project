	...
	<el-main class="bg edit_wrap">
		...
		<el-row class="row_ce">
				...
				<el-form-item label="介绍文档" prop="introduction_document">
					...
					<div v-else>
						<el-upload v-if="(form['exhibition_information_id'] && $check_field('set','introduction_document')) || (!form['exhibition_information_id'] && $check_field('add','introduction_document'))" class="upload-demo" :file-list="introduction_document_FileList" :on-preview="openPreview" :on-remove="introduction_documentRemove" drag
				...
				<el-form-item label="日程安排" prop="schedule_t">
					...
					<ai_editor
						...
						:key="form.exhibition_information_id" 
...
<script>
	...
	export default {
		...
		toFixed: function(num){
			...
			async get_obj_after(json, func){
				if (json.result && json.result.obj) {
					...
					this.event_time_range = this.parseEventTimeRange(obj.event_time);
																																																																								...
																																																																								this.introduction_document_FileList = []
				if (json.result.obj["introduction_document"]) {
				    this.introduction_document_FileList = [
				        {
				            url: this.$fullUrl(json.result.obj['introduction_document']),
				            name: json.result.obj['introduction_document'],
				        },
				    ];
				}
			...
			if(fee_standards_SensitiveWords.length > 0){
				return '费用标准中包含敏感词: ' + fee_standards_SensitiveWords.join(',')
			}
																		var schedule_t_SensitiveWords = await this.filterSensitiveWords(param.schedule_t)
			if(schedule_t_SensitiveWords.length > 0){
				return '日程安排中包含敏感词: ' + schedule_t_SensitiveWords.join(',')
			}
								return null;
			},
