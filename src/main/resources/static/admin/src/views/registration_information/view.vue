<template>
	<el-main class="bg edit_wrap comtable_e">
		...
		<el-row class="row_ce"> 
					...
					<div v-else>
						<el-upload v-if="(form['registration_information_id'] && $check_field('set','enterprise_qualifications')) || (!form['registration_information_id'] && $check_field('add','enterprise_qualifications'))":file-list="enterprise_qualifications_FileList" :on-preview="openPreview" :on-remove="enterprise_qualificationsRemove" class="upload-demo" drag
					...
					<div v-else>
						<el-upload v-if="(form['registration_information_id'] && $check_field('set','exhibitor_documents')) || (!form['registration_information_id'] && $check_field('add','exhibitor_documents'))":file-list="exhibitor_documents_FileList" :on-preview="openPreview" :on-remove="exhibitor_documentsRemove" class="upload-demo" drag
			  ...
			  <el-form-item label="行程确认限次">
				<el-input id="limit_times" v-model="form['travel_confirmation_limit_times']" placeholder="行程确认限制次数，0为不限"
						  v-if="$check_option('/registration_information/view','can_limits')"
				></el-input>
	...
	import mixin from "@/mixins/page.js";
		...
		methods: {
			...
			async get_group_user_enrolled_user() {
							this.form["enrolled_user"] = this.user.user_id;
			...
			async get_obj_after(json, func){
				if (json.result && json.result.obj) {
					const obj = json.result.obj;
																																																																																																								...
																																																																																																								if (this.form["enterprise_qualifications"]) {
					this.enterprise_qualifications_FileList = []
					this.enterprise_qualifications_FileList = [
					  {
						url: this.$fullUrl(this.form['enterprise_qualifications']),
						name: this.form['enterprise_qualifications'],
					  },
					];
				}
																													if (this.form["exhibitor_documents"]) {
					this.exhibitor_documents_FileList = []
					this.exhibitor_documents_FileList = [
					  {
						url: this.$fullUrl(this.form['exhibitor_documents']),
						name: this.form['exhibitor_documents'],
					  },
					];
				}
