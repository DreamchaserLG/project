<template>
	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
			...
			:default-expand-all="isExpandAll" ref="tableRef">
				...
				<el-table-column prop="notice_number" @sort-change="$sortChange" label="通知编号" 				v-if="$check_field('get','notice_number')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["notice_number"] }}
									</template>
					</el-table-column>
					<el-table-column prop="notice_title" @sort-change="$sortChange" label="通知标题" 				v-if="$check_field('get','notice_title')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["notice_title"] }}
									</template>
					</el-table-column>
					<el-table-column prop="notification_time" @sort-change="$sortChange" label="通知时间" 				v-if="$check_field('get','notification_time')" min-width="300">
						<template slot-scope="scope">
					...
					<el-table-column prop="regular_user" @sort-change="$sortChange" label="普通用户" 				v-if="$check_field('get','regular_user')" min-width="300">
						<template slot-scope="scope">
					{{ get_user_regular_user(scope.row['regular_user']) }}
				</template>
					</el-table-column>
					<el-table-column prop="notice_content" @sort-change="$sortChange" label="通知内容" 				v-if="$check_field('get','notice_content')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["notice_content"] }}
									</template>
					</el-table-column>
					<el-table-column prop="mark_status" @sort-change="$sortChange" label="标记状态" 				v-if="$check_field('get','mark_status')" min-width="300">
					<template slot-scope="scope">
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/message_notification/table','set') || $check_action('/message_notification/view','set') || $check_action('/message_notification/view','get')
						" >
			...
			get_list_before(param){
				...
				if(user_group != "管理员"){
						...
						sqlwhere+= "host_user = " + this.user.user_id + " or ";
													...
													if(user_group=="普通用户"){
						sqlwhere+= "regular_user = " + this.user.user_id + " or ";
					}
