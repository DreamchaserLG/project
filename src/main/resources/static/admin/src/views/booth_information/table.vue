	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
			...
			:default-expand-all="isExpandAll" ref="tableRef">
				...
				<el-table-column prop="booth_number" @sort-change="$sortChange" label="展位编号" 				v-if="$check_field('get','booth_number')" min-width="300">
					...
					<el-table-column prop="booth_location" @sort-change="$sortChange" label="展位位置" 				v-if="$check_field('get','booth_location')" min-width="300">
					<template slot-scope="scope">
					...
					<el-table-column prop="booth_specifications" @sort-change="$sortChange" label="展位规格" 				v-if="$check_field('get','booth_specifications')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["booth_specifications"] }}
									</template>
					</el-table-column>
					<el-table-column prop="booth_images" @sort-change="$sortChange" label="展位图片" 				v-if="$check_field('get','booth_images')" min-width="300">
						<template slot-scope="scope">
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/booth_information/table','set') || $check_action('/booth_information/view','set') || $check_action('/booth_information/view','get')
						|| $check_action('/registration_information/table','add') || $check_action('/registration_information/view','add')
						" >
				...
				<template slot-scope="scope">
					...
					v-if="$check_action('/booth_information/table','set') || $check_action('/booth_information/view','set') || $check_action('/booth_information/view','get')"
						...
						<router-link v-if="$check_comment('/booth_information/table')" class="e-button el-button--small is-plain el-button--primary"
								  :to="'../comment/table?size=10&page=1&source_table=booth_information&source_field=' + field + '&source_id=' + scope.row[field]" size="small">
						<span>查看评论</span>
					</router-link>
						<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="changeSigning(scope.row, scope.$index)" v-if="$check_option('/booth_information/table', 'examine')  ">
						<span>审核</span>
					</el-button>
				</div>
				</template>
