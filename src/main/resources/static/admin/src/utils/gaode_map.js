...
// 用户提供的 Key 和 密钥
// Please configure AMap keys in .env.local or the hosting environment.

const AMAP_KEY = process.env.VUE_APP_AMAP_JS_KEY || '';
const AMAP_WEB_KEY = process.env.VUE_APP_AMAP_WEB_KEY || '';
const AMAP_SECURITY_CODE = process.env.VUE_APP_AMAP_SECURITY_CODE || '';

const requireConfig = (value, envName) => {
  if (!value) {
    throw new Error(`请先配置 ${envName}`);
  }
};
...
export const loadAMap = (plugins = [], version = '2.0') => {
  requireConfig(AMAP_KEY, 'VUE_APP_AMAP_JS_KEY');
  requireConfig(AMAP_SECURITY_CODE, 'VUE_APP_AMAP_SECURITY_CODE');
...
export const searchPoiByApi = async (keyword, city = '全国') => {
  if (!keyword) return Promise.resolve([]);
  try {
    requireConfig(AMAP_WEB_KEY, 'VUE_APP_AMAP_WEB_KEY');
