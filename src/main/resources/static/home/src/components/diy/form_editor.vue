<template>
  <div v-if="publish_flag" class="form_editor">
    <div class="editor">
      <!-- 富文本编辑 -->
      <!-- <quill-editor v-model.trim="form.content" class="form_editor_block">
      </quill-editor> -->
      <div
        ref="divRef"
        style="height: 600px"
        v-loading="loading"
        element-loading-text="AI分析中..." />
      <div
        style="display: none"
        v-html="md.render(answerMD)"
        ref="answerRef"></div>
    </div>
    <div class="btn_publish">
      <b-button variant="outline-primary" block @click="publish()"><span>发表</span></b-button>
    </div>
  </div>
</template>

<script>
import mixin from "@/mixins/page.js";
import { AiEditor } from "aieditor";
import { $aiPost } from "@/api/aiApi";
import "aieditor/dist/style.css";
import MarkdownIt from "markdown-it";

export default {
  mixins: [mixin],
  props: {
    form: {
      type: Object,
      default() {
        return {
          content: "",
        };
      },
    },
    details: {
      type: String,
      default: "",
    },
    post_url: {
      type: String,
      default() {
        return "";
      },
    },
    publish_flag:{
      type: Boolean,
      default() {
        return true;
      },
    }
  },
  data() {
    return {
      aiEditor: null,
      // content: null, // Removed duplicate content definition, use form.content or local content synced
      answerMD: "",
      loading: false,
      md: new MarkdownIt(),
      toolbarConfig: {
        excludeKeys: [
          "insertVideo",
          "uploadVideo",
          "group-video",
          "insertImage",
          "insertLink",
          "insertTable",
          "codeBlock",
          "blockquote",
          "todo",
          "emotion",
          "fontSize",
          "fontFamily",
          "lineHeight",
          "group-more-style",
          "color",
          "bgColor",
        ],
      },
    };
  },
  mounted() {
    setTimeout(() => {
      // Use form.content as initial content
      this.initEditor();
    }, 200);
  },
  beforeDestroy() {
    this.aiEditor && this.aiEditor.destroy();
  },
  methods: {
    initEditor() {
      this.aiEditor = new AiEditor({
        element: this.$refs.divRef,
        toolbarExcludeKeys: ["emoji"],
        placeholder: "点击输入内容...",
        content: this.form.content || "",
        ai: {
          menus: [
            {
              icon: '<div style="width:18px;height: 18px;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="none" d="M0 0h24v24H0z"></path><path d="M4 18.9997H20V13.9997H22V19.9997C22 20.552 21.5523 20.9997 21 20.9997H3C2.44772 20.9997 2 20.552 2 19.9997V13.9997H4V18.9997ZM16.1716 6.9997L12.2218 3.04996L13.636 1.63574L20 7.9997L13.636 14.3637L12.2218 12.9495L16.1716 8.9997H5V6.9997H16.1716Z"></path></svg></div>',
              name: "AI 续写",
              prompt: "请帮我继续扩展一些这段话的内容",
              text: "focusBefore",
              model: "auto",
              onClick: () => {
                this.aiHandle("请帮我继续扩展一些这段话的内容：");
              },
            },
            {
              icon: '<div style="width:18px;height: 18px;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="none" d="M0 0h24v24H0z"></path><path d="M15 5.25C16.7949 5.25 18.25 3.79493 18.25 2H19.75C19.75 3.79493 21.2051 5.25 23 5.25V6.75C21.2051 6.75 19.75 8.20507 19.75 10H18.25C18.25 8.20507 16.7949 6.75 15 6.75V5.25ZM4 7C4 5.89543 4.89543 5 6 5H13V3H6C3.79086 3 2 4.79086 2 7V17C2 19.2091 3.79086 21 6 21H18C20.2091 21 22 19.2091 22 17V12H20V17C20 18.1046 19.1046 19 18 19H6C4.89543 19 4 18.1046 4 17V7Z"></path></svg></div>',
              name: "AI 优化",
              prompt: "请帮我优化一下这段文字的内容，并返回结果",
              text: "selected",
              model: "auto",
              onClick: () => {
                this.aiHandle("请帮我优化一下这段文字的内容，并返回结果：");
              },
            },
            {
              icon: '<div style="width:18px;height: 18px;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="none" d="M0 0h24v24H0z"></path><path d="M17.934 3.0359L19.666 4.0359L18.531 6H21V8H19V12H21V14H19V21H17V14L13.9157 14.0004C13.5914 16.8623 12.3522 19.3936 10.5466 21.1933L8.98361 19.9233C10.5031 18.4847 11.5801 16.4008 11.9008 14.0009L10 14V12L12 11.999V8H10V6H12.467L11.334 4.0359L13.066 3.0359L14.777 6H16.221L17.934 3.0359ZM5 13.803L3 14.339V12.268L5 11.732V8H3V6H5V3H7V6H9V8H7V11.197L9 10.661V12.731L7 13.267V18.5C7 19.8807 5.88071 21 4.5 21H3V19H4.5C4.74546 19 4.94961 18.8231 4.99194 18.5899L5 18.5V13.803ZM17 8H14V12H17V8Z"></path></svg></div>',
              name: "AI 校对",
              prompt:
                "请帮我找出这段话的错别字，把错别字修改后，并返回结果，不要解释或其他多余的内容",
              text: "selected",
              model: "auto",
              onClick: () => {
                this.aiHandle(
                  "请帮我找出这段话的错别字，把错别字修改后，并返回结果，不要解释或其他多余的内容："
                );
              },
            },
            {
              icon: '<div style="width:18px;height: 18px;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="none" d="M0 0h24v24H0z"></path><path d="M5 15V17C5 18.0544 5.81588 18.9182 6.85074 18.9945L7 19H10V21H7C4.79086 21 3 19.2091 3 17V15H5ZM18 10L22.4 21H20.245L19.044 18H14.954L13.755 21H11.601L16 10H18ZM17 12.8852L15.753 16H18.245L17 12.8852ZM8 2V4H12V11H8V14H6V11H2V4H6V2H8ZM17 3C19.2091 3 21 4.79086 21 7V9H19V7C19 5.89543 18.1046 5 17 5H14V3H17ZM6 6H4V9H6V6ZM10 6H8V9H10V6Z"></path></svg></div>',
              name: "AI 翻译",
              prompt: "请帮我把这段内容翻译为英语，直接返回英语结果",
              text: "selected",
              model: "auto",
              onClick: () => {
                this.aiHandle("请帮我把这段内容翻译为英语，直接返回英语结果：");
              },
            },
          ],
          bubblePanelEnable: false,
          commands: false,
          translate: false,
        },
        onChange: (aiEditor) => {
          this.valChange(aiEditor.getHtml());
        },
      });
    },
    valChange(val) {
      this.form.content = val;
      this.$emit("update_comment", val);
    },
    aiHandle(prompt) {
      this.loading = true;

      $aiPost({
        messages: [
          {
            role: "user",
            content: prompt + this.aiEditor.getText(),
          },
        ],
      }).then((res) => {
        this.answerMD = res.choices[0].message.content;
        setTimeout(() => {
          // this.content = this.$refs.answerRef.innerHTML;
          this.form.content = this.$refs.answerRef.innerHTML;
          this.aiEditor.setContent(this.$refs.answerRef.innerHTML);
          this.loading = false;
        }, 100);
      });
    },
    /**
     * 发表评论
     */
    async publish() {
      if (!this.user.user_id) {
        this.$router.push("/account/login");
        return
      }
      var form = this.form;
      form = Object.assign(form, {
        user_id: this.user.user_id,
        avatar: this.user.avatar,
        nickname: this.user.nickname,
      });
      if(!form.content){
        this.$toast("输入内容不能为空")
        return
      }

      var sensitiveWords = await this.filterSensitiveWords(form.content)
      if(sensitiveWords.length > 0){
        this.$toast('当前消息中包含敏感词: ' + sensitiveWords.join(','))
        return
      }

      var post_url = this.post_url
      if(!post_url){
        post_url = "~/api/comment/add?"
      }
      this.$post(post_url, form, (res) => {
        if (res.result) {
          this.$emit("update_comment");
          this.form.content = "";
          if (this.aiEditor) {
            this.aiEditor.setContent("");
          }
        } else if (res.error) {
          this.$toast(res.error.message);
        }
      });
    },
  },
  components: {},
};
</script>

<style scoped>
.btn_publish {
  text-align: right;
  margin-top: 10px;
}
.btn_publish button {
  border-radius: 3px;
  cursor: pointer;
}
.btn_publish button:hover {
  background: var(--color_success_b);
  color: white;
}
>>> em,
>>> i {
  font-style: italic !important;
}
>>> u {
  text-decoration: underline !important;
}
>>> s,
>>> del {
  text-decoration: line-through !important;
}
</style>
