<template>
	<div class="diy_details page_exhibition_announcement" id="exhibition_announcement_details">
		...
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_exhibition_announcement'>
							<div_exhibition_announcement :obj="obj"></div_exhibition_announcement>
...
<script>
	import div_exhibition_announcement from "@/components/diy/div_exhibition_announcement.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/exhibition_announcement/set?exhibition_announcement_id=' + obj["exhibition_announcement_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			async get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					console.log(json.result.obj)
														if(json.result.obj["release_time"]=="0000-00-00 00:00:00"){
					  json.result.obj["release_time"] = null;
					}
					if(parseInt(json.result.obj["release_time"]) > 9999){
						json.result.obj["release_time"] = this.$toTime(parseInt(json.result.obj["release_time"]),"yyyy-MM-dd hh:mm:ss")
					}
					...
					this.form_comment.source_id = this.obj.exhibition_announcement_id;
												}
			},
											/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "exhibition_announcement",
					source_field: "exhibition_announcement_id",
					source_id: obj["exhibition_announcement_id"],
					user_id
				};
				...
				if (this.state_collect) {
					...
					this.$get('~/api/collect/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/exhibition_announcement/set?exhibition_announcement_id=' + _this.obj["exhibition_announcement_id"], {
								collect
							}, (res) => {
								if(res.result){
...
.page_exhibition_announcement .warp,
.page_exhibition_announcement .warp_dt_box,
.page_exhibition_announcement .container {
  padding: 0;
}

.page_exhibition_announcement .card_exhibition_announcement,
.page_exhibition_announcement .comment_box,
.page_exhibition_announcement .form_edit_box {
  border-radius: 28px;
...
.page_exhibition_announcement .review {
  margin-top: 1.3rem;
}
...
.page_exhibition_announcement .details_form_editor_title {
