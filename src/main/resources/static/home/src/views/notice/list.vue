    ...
    <section class="page_query_card">
      <div class="page_query_grid">
        <div class="page_query_item wide notice_query_main">
          <label>公告标题</label>
          <b-form-input v-model.trim="query.title" placeholder="请输入公告标题"></b-form-input>
        </div>
        <div class="page_query_actions">
          <button type="button" class="page_primary_btn" @click="search()">查询</button>
          <button type="button" class="page_secondary_btn" @click="resetNotice">重置</button>
        </div>
      </div>
    </section>
      ...
      <div v-if="list.length" class="notice_rows">
        ...
        >
          <div class="notice_row_media">
            <img :src="getNoticeCover(index)" alt="notice cover">
            <span class="notice_row_badge">平台公告</span>
          </div>

          <div class="notice_row_body">
            <div class="notice_row_top">
              <div class="notice_row_code">{{ getNoticeCode(item) }}</div>
              <div class="notice_row_time">{{ formatDateTime(item.create_time) }}</div>
            </div>

            <h3 class="notice_row_title">{{ item.title }}</h3>
            <p class="notice_row_desc">{{ excerpt(item.content, 150) }}</p>

            <div class="notice_row_footer">
              <span class="notice_row_type">{{ getNoticeType(item) }}</span>
...
export default {
  ...
  methods: {
    ...
    getNoticeCover(index) {
      return this.coverList[index % this.coverList.length];
    },
    getNoticeCode(item) {
      const value = String(item.notice_id || 0).padStart(4, "0");
      return `PTGG-${value}`;
    },
    getNoticeType(item) {
