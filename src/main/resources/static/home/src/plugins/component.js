...
import ai_editor from "@/components/diy/ai_editor.vue";
...
export default {
	 ...
	 */
	install(Vue,optons) {
		Vue.component("ai_editor", ai_editor);
		// 富文本编辑器实例化
		...
		Vue.mixin({
			...
			methods: {
				...
				/**
				 * 通用日期时间格式化（支持 Date/时间戳/字符串）
				 * @param {Date|Number|String} date 输入值：Date、时间戳（秒或毫秒）、字符串
				...
				$formatTime(date, format = 'yyyy-MM-dd hh:mm:ss') {
					...
					if (value instanceof Date) {
						return value.toStr(format);
					}
					// 数字：自动识别秒/毫秒时间戳
					if (typeof value === 'number') {
						var ts = value < 1e11 ? value * 1000 : value; // 10^11 作为分界
						return new Date(ts).toStr(format);
					}
					// 纯数字字符串：当作时间戳
					if (typeof value === 'string' && /^\d+$/.test(value)) {
						var n = Number(value);
						var ts2 = value.length <= 10 ? n * 1000 : n;
						return new Date(ts2).toStr(format);
					}
					// 仅时间字符串（HH:mm 或 HH:mm:ss），直接返回或按目标格式输出
					if (typeof value === 'string' && /^(\d{2}):(\d{2})(:(\d{2}))?$/.test(value)) {
						// 如果目标格式只包含时间部分，直接返回；否则拼入今日日期
						var onlyTimeFmt = /h+|H+/.test(format) && !/y+|M+|d+/.test(format);
						if (onlyTimeFmt) return value;
						var now = new Date();
						var parts = value.split(':');
						now.setHours(Number(parts[0]));
						now.setMinutes(Number(parts[1]));
						now.setSeconds(parts[2] ? Number(parts[2]) : 0);
						return now.toStr(format);
					}
					// 其他字符串：用已有 toTime 能力解析
					try {
						return String(value).toTime().toStr(format);
					} catch(e) {
						return '';
					}
				},
				...
				$get_auth(user_group,func){
					let orderby = "auth_id asc";
					// 查询权限
					this.$get("~/api/auth/get_list?like=0", {
						user_group,
						orderby
					}, (json) => {
				...
				$hide_str(content) {
					...
					if (str.includes("@")) {
						...
						return this.$hide_str(parts[0]) + "@" + parts.slice(1).join("@");
					}
					...
					if (len === 2) {
						...
						return str.substring(0, 3) + "*".repeat(len - 7) + str.substring(len - 4);
					}
					return str;
				},
