...
public class UserController extends BaseController<User, UserService> {
    private static final Duration ACCESS_TOKEN_DURATION = Duration.ofSeconds(7200L);
    ...
    public Map<String, Object> login(@RequestBody Map<String, String> data, HttpServletRequest httpServletRequest) {
        ...
        if (byUsername.getState()!=1){
            return error(30000,"用户非可用状态，不能登录");
                ...
                accessToken.setMaxage((int) ACCESS_TOKEN_DURATION.getSeconds());
                saveAccessToken(accessToken, ACCESS_TOKEN_DURATION);
    ...
    public Integer tokenGetUserId(String token) {
        ...
        AccessToken byToken = getAccessToken(token);
        if(byToken == null){
            return 0;
        }
        if (isAccessTokenExpired(byToken)) {
            deleteAccessToken(token);
            return 0;
        }
    ...
    private AccessToken getAccessToken(String token) {
        try {
            return (AccessToken) redisTemplate.opsForValue().get(token);
        } catch (Exception e) {
            log.warn("Redis unavailable while loading token, fallback to database: {}", e.getMessage());
        }
    ...
    private void deleteAccessToken(String token) {
        if (token == null || "".equals(token)) {
            return;
        }
        ...
        try {
            ...
            log.warn("Redis unavailable while deleting token, fallback to database: {}", e.getMessage());
        }
    ...
    private boolean isAccessTokenExpired(AccessToken accessToken) {
        ...
        Integer maxage = accessToken.getMaxage();
        if (maxage == null || maxage <= 0) {
            return false;
        }

        Timestamp baseTime = accessToken.getUpdateTime();
        if (baseTime == null) {
            baseTime = accessToken.getCreateTime();
        }
        if (baseTime == null) {
            return false;
        }

        long expireAt = baseTime.getTime() + maxage * 1000L;
