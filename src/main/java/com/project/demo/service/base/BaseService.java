...
public class BaseService<E>{
    ...
    @Transactional
    public void updateEntity(Map<String,String> query,Map<String,String> config,E entity){
        QueryWrapper wrapper = new QueryWrapper<E>();
        toWhereWrapper(query,"0".equals(config.get(FindConfig.LIKE)),wrapper);
        baseMapper.update(entity,wrapper);
        log.info("[{}] - 更新操作：{}",entity);
...
//    }
    ...
    public String select(Map<String,String> query,Map<String,String> config){
        ...
        sql.append("from ").append("`").append(table).append("`").append(toWhereSql(query, "1".equals(config.get(FindConfig.LIKE)),config.get(FindConfig.SQLHWERE)));
        if (config.get(FindConfig.GROUP_BY) != null && !"".equals(config.get(FindConfig.GROUP_BY))){
            sql.append("group by ").append(config.get(FindConfig.GROUP_BY)).append(" ");
        }
        if (config.get(FindConfig.ORDER_BY) != null && !"".equals(config.get(FindConfig.ORDER_BY))){
...
//        log.info("拼接统计函数前");
        if (config.get(FindConfig.GROUP_BY) != null && !"".equals(config.get(FindConfig.GROUP_BY))){
            ...
            sql.append(toWhereSql(query, "1".equals(config.get(FindConfig.LIKE)),config.get(FindConfig.SQLHWERE)));
//            sql.append(" ").append("GROUP BY ").append(config.get(FindConfig.GROUP_BY));
        }else {
...
//    }
    ...
    public Map<String,String> readConfig(HttpServletRequest request){
        ...
        String likeParam = request.getParameter(FindConfig.LIKE);
        if (likeParam == null || likeParam.trim().isEmpty()) {
            map.put(FindConfig.LIKE, "1");
        } else {
            map.put(FindConfig.LIKE, likeParam);
        }
